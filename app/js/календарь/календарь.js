...
// Добавил сам - изменение текста в поле
calendarText.forEach(function(item, index){
	if (calendar[index].value != '') {
		item.innerHTML = '<b>' + calendar[index].value + '</b>'
	} 
})
window.addEventListener('click', function(event) {
	if (!event.target.closest('.xcalend') && 
		!event.target.closest('.datepicker') && 
		!event.target.closest('.cal-l') && 
		!event.target.closest('.cal-r') && 
		!event.target.closest('td') 
		)  {
		document.querySelector('#xcalend').style.display="none";
	}
})
...
Nod(a.id).style.top=oo.bottom+scrol()+3+"px";
...
xCal.fM = function(a) {
	...
	HTM(a.id, '<tbody><tr><th rowspan=4 valign=bottom><a href="#">?</a></th>'+ca+'</tr></tbody>');
